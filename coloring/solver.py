...
def solve_it(input_data):
    ...
    # solution_new = [0 for i in range(node_count)]
    # for node in best_solution_nodes:
    #     solution_new[node.index] = node.color[0]
    # print(solution_new)
    # color_result_list = []
    ...
    with open('/home/kaylor/Desktop/discrete_optimization/coloring/answer.txt', 'r') as answer_data_file:
        answer_data = answer_data_file.read()
    lines = answer_data.split('\n')
    # for line in lines:
    #     print(line)
    # exit(1)
    solution_new = []
    current_opt = int(lines[10])
    parts = lines[11].split(' ')
    for i in range(len(parts)):
        solution_new.append(int(parts[i]))
